...
if model.status == GRB.OPTIMAL:
    ...
    uav_schedule = {f"uav_{i+1}": [] for i in range(n)}
    for uav_idx in range(n):
        offset = int(sum(T[:uav_idx]))
        uav_schedule[f"uav_{uav_idx+1}"] = [int(x_values[f"r[{offset + t}]"]) for t in range(int(T[uav_idx]))]

    with open("uav_schedule.json", "w") as f:
        json.dump(uav_schedule, f)
